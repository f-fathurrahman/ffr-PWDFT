SUBROUTINE my_gencore()
  USE m_constants, ONLY: y00, solsc
  USE m_atoms, ONLY: natmmax, nspecies, idxas, natoms
  USE m_atomic_species, ONLY: nrspmax, nstspmax, nstsp, spcore, &
                 vrsp, nrspmax, nrsp, rsp, nsp, lsp, ksp
      ...
      DO ispn = 1,nspncr
        ...
        DO ist = 1,nstsp(is)
          IF( spcore(ist,is) ) THEN 
            ...
            DO ir = 1,nr
              rhocr(ir,ias,ispn) = rhocr(ir,ias,ispn) + t1*( rwfcr(ir,1,ist,ias)**2 + rwfcr(ir,2,ist,ias)**2 )
            ENDDO 
          ENDIF 
        ENDDO 
        DO ir = 1,nr
