...
!-------------------------
  ...
  CALL r3mtm(avec,avec,g) ! avec dependence enters via g
  !
  ! loop over all possible symmetry matrices
  nsymlat = 0
  !
  ! TODO: Make this loop over a list of predetermined matrix
  !       Include only the possible choices of symmetry matrix
  ...
  DO i31 = -1,1; DO i32 = -1,1; DO i33 = -1,1
    ...
    IF(abs(md) /= 1) goto 10 ! skip this matrix
    !
    ! check invariance of metric tensor
    s(:,:) = dble(sym(:,:)) ! convert to real(8)
    CALL r3mtm(s, g, c) ! compute S^T * S
    CALL r3mm(c, s, sgs)
    DO j = 1,3
      DO i = 1,3
        IF(abs(sgs(i,j)-g(i,j)) > epslat) goto 10 ! next matrix
      ENDDO 
    ENDDO 
    !
    ! Additional checks for some special cases: spin-spiral, electric field, A-field
    !
